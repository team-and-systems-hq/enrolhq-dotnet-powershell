...
try {
    $reportData = $applications | ForEach-Object {
        [PSCustomObject]@{
            ApplicationId     = $_.id
            StudentFirstName  = $_.first_name
            StudentLastName   = $_.last_name
            EntryYear         = $_.entry_year
            EntryGrade        = $_.entry_grade
            Status            = $_.application_status
            ParentName        = "$($_.user_parent.first_name) $($_.user_parent.last_name)"
...
try {
    ...
    if ($format -eq 'csv') {
        $csvContent = $applications |
            Select-Object id, first_name, last_name, entry_year,
                entry_grade, application_status, created_at |
