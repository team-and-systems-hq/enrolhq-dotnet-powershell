...
/// Usage:
/// <code>
/// using var client = new EnrolHQClient("yourschool", "your_api_token");
...
public sealed class EnrolHQClient : IDisposable
{
    ...
    /// <summary>Create a client using an instance name (e.g. "yourschool").</summary>
    public EnrolHQClient(string instance, string apiToken, TimeSpan? timeout = null, int maxRetries = 3)
        : this($"https://{instance}.enrolhq.com.au/api/v2/", apiToken, timeout, maxRetries, isBaseUrl: true)
    {
    }
    ...
    public static EnrolHQClient FromEnvironment(TimeSpan? timeout = null, int maxRetries = 3)
    {
        ...
        var instance = Environment.GetEnvironmentVariable("ENROLHQ_INSTANCE")
            ?? throw new InvalidOperationException(
                "Either ENROLHQ_BASE_URL or ENROLHQ_INSTANCE environment variable is required");

        return new EnrolHQClient(instance, apiToken, timeout, maxRetries);
    }
