...
public sealed class EnrolHQHttpClient : IDisposable
{
    ...
    /// <summary>
    /// Fetches all pages for a paginated endpoint and returns the combined results.
    /// </summary>
    /// <param name="maxPages">
    /// Safety limit to prevent unbounded memory usage. At the default page size of 100,
    /// the limit of 100 pages allows up to 10,000 records. Increase if needed.
    /// </param>
    public async Task<List<T>> GetAllPagesAsync<T>(string endpoint,
        Dictionary<string, string?>? queryParams = null, int pageSize = 100, int maxPages = 100,
        CancellationToken cancellationToken = default)
    {
