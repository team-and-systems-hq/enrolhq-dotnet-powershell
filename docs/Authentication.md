...
```bash
# Bash / Zsh
export ENROLHQ_INSTANCE='yourschool'
export ENROLHQ_API_TOKEN='ekt_live_...'

# Windows Command Prompt
set ENROLHQ_INSTANCE=yourschool
...
```powershell
Connect-EnrolHQ -Instance 'yourschool' -ApiToken 'ekt_live_...'
```
...
Connect-EnrolHQ -Instance 'yourschool' -ApiToken $secret
```
...
2. **Use `SecureString`** when passing tokens in PowerShell to prevent them from
   ...
   Connect-EnrolHQ -Instance 'yourschool' -ApiToken $secureToken
   ```
...
Connect-EnrolHQ -Instance 'otherschool' -ApiToken $otherToken
```
